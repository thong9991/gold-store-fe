...
import { deleteCashFlow } from '@/services/cashFlows';
...
const DeleteForm: React.FC<FormProps> = ({
  ...
  return data.table === 'product' ? (
    <form className="p-4 flex flex-col gap-4">
      <span className="text-center font-medium">Tất cả dữ liệu sẽ bị mất, bạn có chắc muốn xóa sản phẩm</span>
      <button
    ...
    <form className="p-4 flex flex-col gap-4" onSubmit={handleSubmit(onSubmit)}>
      <input type="text | number" {...register('id')} defaultValue={data.id} hidden />
      <input type="text" {...register('table')} defaultValue={data.table} hidden />
